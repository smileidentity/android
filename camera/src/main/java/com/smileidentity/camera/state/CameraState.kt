...
import com.smileidentity.camera.extensions.isImageAnalysisSupported
import java.io.File
...
class CameraState(context: Context) {
    ...
    internal var isImageAnalysisEnabled: Boolean = isImageAnalysisSupported
        set(value) {
            ...
            if (!isImageAnalysisSupported) {
                Timber.e("Image analysis is not supported")
                return
            }
            field = value
            updateCaptureMode()
    ...
    private fun updateImageAnalyzer(analyzer: ImageAnalysis.Analyzer? = imageAnalyzer) =
        with(controller) {
    ...
    @RequiresApi(VERSION_CODES.M)
    fun pauseRecording() {
        Timber.i("Pause recording")
        recordController?.pause()
    }

    @RequiresApi(VERSION_CODES.M)
    fun resumeRecording() {
        Timber.i("Resume recording")
...
private fun OutputSize?.toImageTargetSize(): ImageTargetSize? = this?.let {
    if (it.aspectRatio !=
